#!/usr/bin/env python3
"""E1固定规则全200题离线重放；12题看图，不新增任何模型调用或正式QA。"""
import os
...
QUOTA_SOURCE=ROOT/'outputs/analysis/density_reward_decay_200_20260922_r1'
CASES=('103-1','314-3','383-2','414-1','821-3','278-1','260-2','004-3','102-2','736-2','842-3','395-2')
...
def main(run_id):
    ...
    protocol=dict(local_rule='L*sqrt(u)',global_rule='frozen D1 quotas, no reallocation',quota_protocol_sha256=sha256(QUOTA_SOURCE/'protocol.json'),config=cfg,source_protocol_sha256=sha256(SOURCE/'protocol.json'),
        questions=[r['question_id'] for r in manifest['rows']],visual_review=list(CASES),new_model_calls=0,
        code_hashes={str(p.relative_to(ROOT)):sha256(p) for p in (Path(__file__),ROOT/'src/videoqa_audit/local_distance_scale.py')},
        caveat='Fixed D1 quotas; normalized clipped Euclidean distance sqrt(1-maxcos) replaces its square. One diagnostic scale, no exponent search, not an integrated new allocation algorithm, no QA')
    durable(out/'protocol.json',protocol);hashes={};records={};started=time.perf_counter()
    # 步骤1：复现全部原soft输入，消除环境或特征差异，再计算唯一E1候选。
    for ordinal,row in enumerate(manifest['rows']):
        ...
        d1path=QUOTA_SOURCE/'records'/f'{q}.json';d1=read_json(d1path);hashes[str(d1path.relative_to(ROOT))]=sha256(d1path)
        control,control_trace=select_density_decay(s['candidates'],f,s['plan'],s['video'],cfg)
        assert control==d1['selected_indices'] and control_trace['region_budgets']==d1['trace']['region_budgets']
        quotas=d1['trace']['region_budgets'];fixed_control,_=supported_selection(s['candidates'],f,s['plan'],s['video'],cfg,quotas)
        assert fixed_control==control
        ids,trace=select_local_distance(s['candidates'],f,s['plan'],s['video'],cfg,quotas)
        assert trace['region_budgets']==quotas
        ...
        record=dict(question_id=q,cohort='old50' if q in old else 'new150',selected_indices=ids,
            ...
            changed_vs_D1=len(set(ids)-set(control)),budgets_D1=quotas,changed_vs_soft=len(set(ids)-set(baseline)),changed_vs_v1=len(set(ids)-set(s['selected_indices'])),
    ...
    # 步骤2：12题正式旧输入和E1输入并排，图像仅CPU处理，不执行视觉编码器。
    ...
    page=['<!doctype html><meta charset="utf-8"><style>body{font:16px/1.7 sans-serif;max-width:1200px;margin:auto}img{max-width:100%}pre{white-space:pre-wrap}</style><h1>E1：固定D1名额，局部L×sqrt(u)（离线，无问答）</h1><p>上游、锚点与总预算不变。不是正式问答成绩；不得用旧答案给E1计分。</p>']
    for q in CASES:
        a=read_json(SOURCE/'combined_results/v1'/f'{q}.json');r=records[q]
        render(out/'boards'/f'{q}.png',r['selected_frames'],a['selection']['video'],processor,q+' E1 offline; NO QA',{f['source_pts'] for f in r['selected_frames']})
        page.append(f'<details open id="{q}"><summary>{q}：相对soft换{r["changed_vs_soft"]}帧</summary><pre>{html.escape(a["question"])}</pre><p>D1冻结配额{r["budgets_D1"]}；E1配额{r["trace"]["region_budgets"]}</p>')
        for v in ('soft',):
            page.append(f'<h3>{v}正式历史输入</h3><img loading="lazy" src="{os.path.relpath(SOURCE/"combined_boards"/v/(q+".png"),out)}">')
        page.append(f'<h3>D1冻结预算及排序对照</h3><img loading="lazy" src="{os.path.relpath(QUOTA_SOURCE/"boards"/(q+".png"),out)}">')
        page.append(f'<h3>E1诊断输入，未调用问答</h3><a href="records/{q}.json">全部提议/得分/预算</a><img loading="lazy" src="boards/{q}.png"></details>')
    (out/'index.html').write_text(''.join(page))
    # 步骤3：只汇总输入变化，不把任何历史正确率移植为E1成绩。
    summary=dict(questions=200,original_soft_replays_passed=200,new_model_calls=0,qa_status='not_run',
        changed_vs_D1=sum(r['changed_vs_D1']>0 for r in records.values()),changed_slots_vs_D1=sum(r['changed_vs_D1'] for r in records.values()),
        fixed_quota_checks_passed=200,changed_vs_soft=sum(r['changed_vs_soft']>0 for r in records.values()),changed_slots_vs_soft=sum(r['changed_vs_soft'] for r in records.values()),
        ...
        cases={q:{k:records[q][k] for k in ('selected_indices','changed_vs_D1','changed_vs_soft','changed_vs_v1')} for q in CASES},
