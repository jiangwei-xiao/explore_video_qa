#!/usr/bin/env python3
"""P1固定规则全200题离线重放；六题看图，不新增任何模型调用或正式QA。"""
import os
...
CASES=('103-1','314-3','383-2','414-1','821-3','278-1')
...
def main(run_id):
    ...
    protocol=dict(local_rule='L*u',global_rule='w*u',config=cfg,source_protocol_sha256=sha256(SOURCE/'protocol.json'),
        questions=[r['question_id'] for r in manifest['rows']],visual_review=list(CASES),new_model_calls=0,
        code_hashes={str(p.relative_to(ROOT)):sha256(p) for p in (Path(__file__),ROOT/'src/videoqa_audit/priority_decoupling.py')},
        caveat='Two-level attribution heuristic; not claimed as greedy optimization of a single proven set objective; no QA and no parameter search')
    durable(out/'protocol.json',protocol);hashes={};records={};started=time.perf_counter()
    # 步骤1：复现全部原soft输入，消除环境或特征差异，再计算唯一P1候选。
    for ordinal,row in enumerate(manifest['rows']):
        ...
        ids,trace=select_decoupled(s['candidates'],f,s['plan'],s['video'],cfg)
    ...
    # 步骤2：六题正式旧输入和P1输入并排，图像仅CPU处理，不执行视觉编码器。
    ...
    page=['<!doctype html><meta charset="utf-8"><style>body{font:16px/1.7 sans-serif;max-width:1200px;margin:auto}img{max-width:100%}pre{white-space:pre-wrap}</style><h1>P1：区域内L×u，跨区w×u（离线，无问答）</h1><p>上游、锚点与总预算不变。不是正式问答成绩；不得用旧答案给P1计分。</p>']
    for q in CASES:
        a=read_json(SOURCE/'combined_results/v1'/f'{q}.json');r=records[q]
        render(out/'boards'/f'{q}.png',r['selected_frames'],a['selection']['video'],processor,q+' P1 offline; NO QA',{f['source_pts'] for f in r['selected_frames']})
        page.append(f'<details open id="{q}"><summary>{q}：相对soft换{r["changed_vs_soft"]}帧</summary><pre>{html.escape(a["question"])}</pre><p>v1配额{r["budgets_v1"]}；soft配额{r["budgets_soft"]}；P1配额{r["trace"]["region_budgets"]}</p>')
        for v in ('v1','soft'):
            page.append(f'<h3>{v}正式历史输入</h3><img loading="lazy" src="{os.path.relpath(SOURCE/"combined_boards"/v/(q+".png"),out)}">')
        page.append(f'<h3>P1诊断输入，未调用问答</h3><a href="records/{q}.json">全部提议/得分/预算</a><img loading="lazy" src="boards/{q}.png"></details>')
    (out/'index.html').write_text(''.join(page))
    # 步骤3：只汇总输入变化，不把任何历史正确率移植为P1成绩。
