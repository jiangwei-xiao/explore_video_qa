"""E1固定200题独立执行：D1分配配额，局部距离排序；历史结果只读。"""
import contextlib
import fcntl
...
from .e1_pipeline import DensityScorer,prepare_density
from .e1_validation import verify_selection
...
def protocol():
    """冻结完整两遍流程、200次问答上限及离线对照，不把离线配额直接用于正式选择。"""
    from .density_extension import protocol as previous_protocol
    prior=previous_protocol()
    offline=ROOT/'outputs/analysis/local_distance_scale_200_20260922_r1'
    original=read_json(offline/'protocol.json')
    for name,digest in original['code_hashes'].items():
        if sha256(ROOT/name)!=digest:raise ProtocolError('Offline selector changed: '+name)
    files=[ROOT/p for p in ('src/videoqa_audit/e1_pipeline.py','src/videoqa_audit/e1_run.py',
        'src/videoqa_audit/e1_validation.py','src/videoqa_audit/e1_report.py','scripts/run_density_e1.py',
        'src/videoqa_audit/density_reward_decay.py','src/videoqa_audit/local_distance_scale.py')]
    sources={**prior['source_results'],**read_json(offline/'summary.json')['source_hashes']}
    for p in (offline/'records').glob('*.json'):sources[str(p.relative_to(ROOT))]=sha256(p)
    sources[str((offline/'protocol.json').relative_to(ROOT))]=sha256(offline/'protocol.json')
    for name,digest in sources.items():
        if sha256(ROOT/name)!=digest:raise ProtocolError('Frozen source changed: '+name)
    return dict(version='density-e1-two-pass-200-v1',question_count=200,maximum_answer_calls=200,
        variants=list(VARIANTS),scope_calls=0,query_calls=0,automatic_generation_retries=0,
        config=prior['config'],inference=prior['inference'],
        selection='raw scan/refinement -> D1 L*u allocation with 1+(w-1)/n -> fixed quota L*sqrt(u) reselection',
        comparison='historical uniform/topk/v1/soft; report old50/new150/all200 separately',
        source_results=sources,code_sha256={**prior['code_sha256'],**{str(p.relative_to(ROOT)):sha256(p) for p in files}},
        timing='raw processing to answer parsed; both allocation and reselection, checks/checkpoints included; model load/warmup and final persistence excluded')
...
def validate(run,record,row,fingerprint):
    ...
    verify_selection(s,features,read_json(run/'protocol.json')['config'],row['question_id'])
...
def verify_pair(run,qid):
    """单题完成标记；正式候选/配额/最终帧已在生成前与冻结离线结果核验。"""
    r=read_json(run/'results/e1'/f'{qid}.json')
    return dict(question_id=qid,status='passed',selected_indices=r['selection']['selected_indices'])
...
def run_variant(run,row,variant,gpu,backend,scorer,fingerprint):
    ...
    if resumed:
        ...
        selection,features,timing=prepare_density(ROOT/'data'/row['video_relative_path'],row['question'],scorer,cfg,run/'fine_frames'/key)
    ...
    verify_selection(selection,features,cfg,qid)
...
def check_recovery(run,rows,fingerprint):
    """200次固定调用ID检查；完成结果重放，不确定生成一律暂停。"""
    ids={f'{r["question_id"]}__{v}' for r in rows for v in VARIANTS}
    calls=list((run/'calls/answer').glob('*.json'));assert len(calls)<=200
    for p in calls:
...
def execute(run_id, requested_gpus, resume=False, prepare_only=False):
    """后台批量接口：冻结指纹、排他锁、前5题验收后继续195题，完成条目不重答。"""
    ...
    with (run/'coordinator.lock').open('a') as lock:
        ...
        for name in ('results/e1','pairs','calls/answer','checkpoints','features','fine_frames','workers','failures','logs','persistence','code_snapshot'):
        ...
        if len(rows)!=200 or len({r['question_id'] for r in rows})!=200:
            raise ProtocolError('Expected frozen new200 questions')
        completed=len(check_recovery(run,rows,fingerprint))
        if completed==200:
            print('Verified 200 completed E1 answers; no models loaded and no generation repeated.',flush=True);return run
        if prepare_only:
            durable(run/'status.json',dict(status='prepared_not_started',new_questions=200,maximum_answer_calls=200,utc=utc()))
            return run
        ...
        try:
            publish();verification=preflight(rows,dict(question_count=200,unique_video_count=200,manifest_path=str(MANIFEST.relative_to(ROOT)),manifest_sha256=sha256(MANIFEST),asset_audit_path='data/manifests/video_mme_full_asset_audit.json'));durable(run/'preflight.json',verification)
            ...
            if len(check_recovery(run,rows,fingerprint))!=200:
