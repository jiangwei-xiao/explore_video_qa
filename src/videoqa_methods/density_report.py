...
def render_board(path, images, selected, processor, qid):
    ...
    draw.text((8,8),f'{qid} Density-v1: ordered 384x384 QA input',fill='black')
...
def summarize(run):
    ...
    for row in rows:
        ...
        if not (run/'boards'/f'{q}.png').exists():
            ...
            for image in images:
                image.close()
    methods={'Density-v1':current,**{name:{q:read_json(path/f'{q}.json') for q in ids} for name,path in HISTORY_PATHS.items()}}
    ...
    for q in ids:
        ...
        mechanisms.append(dict(question_id=q,regions=len(s['plan']['regions']),cores=len(s['plan']['cores']),
            ...
            zero_diversity_fill=sum(t['zero_diversity_fill'] for t in s['selection_trace']['rounds']),
    ...
    lines=['# 检索密度v1：开发50题初版结果','',f'运行：`{run.name}`。冻结参数单版本；50题均为已观察开发集，旧方法不重新生成。','',
           '| 方法 | 总体 | 短 | 中 | 长 |','|---|---:|---:|---:|---:|']
    ...
    a=summary['accuracy']['Density-v1']['all'];comp=summary['comparisons']['TopK']
    ...
    blocks=['<!doctype html><meta charset="utf-8"><style>body{font:16px/1.7 sans-serif;max-width:1100px;margin:auto}img{max-width:100%}pre{white-space:pre-wrap}</style><h1>检索密度v1：50题输入与结果</h1>']
    for row,m in zip(rows,mechanisms):
