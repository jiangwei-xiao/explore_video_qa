...
def summarize(run_dir):
    ...
    for method in METHODS:
        ...
        if len(paths) != 50:
            raise ValueError(f'{method} has {len(paths)} results, expected 50')
        for p in paths:
            r = read_json(p)
    ...
    for row in rows:
        validate_pair({m: records[m][row['question_id']] for m in METHODS}, row, protocol_hash)
    methods = {}
    for method in METHODS:
    ...
    report = dict(run_id=run_dir.name, protocol_sha256=protocol_hash, results=100, methods=methods,
    ...
    lines = [f'# Video-MME 50题基线：{run_dir.name}', '',
             '两组各50题、100条新结果；旧单题验收未混入。以下为开发集结果。', '',
    ...
    for method in METHODS:
        ...
        lines.append(f'| {method} | {a["correct"]}/50 | {a["accuracy"]:.1%} | {interval[0]:.1%}–{interval[1]:.1%} | '
    ...
    for label, ids in cells.items():
        lines += ['', f'- {label}：{len(ids)}题；' + ', '.join(ids)]
    lines += ['', f'调度器总墙钟时间（含核验、模型加载、合成预热、调度与保存）：{execution["total_active_wall_seconds"] / 60:.2f}分钟。',
              ...
              '工作进程同时常驻LLaVA和BLIP，总显存不等于均匀方法独立部署显存；阶段增量与分组详细分位数见summary.json。结果不能外推为完整Video-MME成绩或稳定方法优势。', '']
