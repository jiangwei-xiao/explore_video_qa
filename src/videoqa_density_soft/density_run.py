...
HISTORY_PATHS = dict(HISTORY_PATHS, Density_v1=ROOT/'outputs/methods/retrieval_density_v1_videomme50_20260920_r1/results')
...
def execution_protocol():
    ...
    # 步骤1：正式版本不能带入离线扩窗条件，v1配置保持只读。
    from videoqa_audit.density_local_validation import spec as local_spec
    local_spec()
    files = sorted((ROOT/'src/videoqa_runtime').glob('*.py')) + sorted((ROOT/'src/videoqa_methods').glob('*.py'))
    files += [ROOT/'scripts/run_density_soft.py', CONFIG, ROOT/'configs/local_model_inventory.json',
    ...
    files += sorted((ROOT/'src/videoqa_density_soft').glob('*.py')) + [ROOT/'src/videoqa_audit/density_local_validation.py']
    history = [p for directory in HISTORY_PATHS.values() for p in sorted(directory.glob('*.json'))]
    return dict(version='retrieval-density-soft-v1',selection_rule='max seed visual support times novelty; joint region competition; fixed radius 4s',config=read_json(CONFIG),question_count=50,
...
def verify_selection(selection, features, cfg, historical=None):
    ...
    from videoqa_methods.density_selection import refinement_targets, source_time
...
def validate_result(run, record, row, fingerprint, verify_assets=False):
    ...
    if record['status']!='completed' or record['method']!='density_soft' or record['protocol_sha256']!=fingerprint:
...
def verify_upstream(qid, selection, features):
    """正式问答前核对v1上游身份；只允许最终竞争变化，不按答案决定继续。"""
    old_run=ROOT/'outputs/methods/retrieval_density_v1_videomme50_20260920_r1'
    old=read_json(old_run/'results'/f'{qid}.json'); source=old['selection']
    # 步骤1：离散上游与固定四秒候选池必须相同，资产路径允许属于新运行。
    for key in ('video','candidates','initial_count','plan','refinement','new_candidate_count'):
        if selection[key]!=source[key]:
            raise ProtocolError(f'Upstream differs from v1: {qid} {key}')
    # 步骤2：原评分和全池视觉特征允许1e-6绝对误差，不重新调用模型修正。
    if not np.allclose(selection['initial_scores'],source['initial_scores'],atol=1e-6,rtol=0):
        raise ProtocolError('Upstream score drift')
    old_features=np.load(old_run/old['feature_file'],allow_pickle=False)
    if features.shape!=old_features.shape or not np.allclose(features,old_features,atol=1e-6,rtol=0):
        raise ProtocolError('Upstream visual feature drift')
    # 步骤3：实际选择还须与冻结v1特征下的同规则重放完全一致。
    expected,_=select_joint(source['candidates'],old_features,source['plan'],source['video'],read_json(CONFIG))
    if expected!=selection['selected_indices']:
        raise ProtocolError('Soft discrete selection differs from frozen-pool replay')
...
def run_question(run, row, gpu, backend, scorer, fingerprint):
    ...
    verify_upstream(qid,selection,features)
    ...
    result=dict(status='completed',method='density_soft',question_id=qid,video_id=row['video_id'],stratum=row['stratum'],question=row['question'],options=row['options'],
